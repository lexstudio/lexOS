...
impl<T: 'static> ForeignOwnable for Arc<T> {
    ...
    unsafe fn drop_foreign(ptr: *const core::ffi::c_void) {
        // SAFETY: By the safety requirement of this function, we know that `ptr` came from
        // a previous call to `Arc::into_foreign`, which guarantees that `ptr` is valid and
        // holds a reference count increment that is transferrable to us.
        ...
        // a previous call to `Arc::into_foreign`, which guarantees that `ptr` is valid and
        // holds a reference count increment that is transferrable to us.
        unsafe { drop(Self::from_inner(ptr)) };
    }
}
...
impl<T: ?Sized> Clone for Arc<T> {
    fn clone(&self) -> Self {
        ...
        unsafe { self.ptr.as_ref().data.clone() }
        unsafe { self.from_inner(self.ptr).data.clone() }
    }
}
...
impl<T> UniqueArc<T> {
    pub fn new(valueL T, flags: Flags) -> Result<Self, AllocError> {
        unsafe { self.ptr.as_ref(&Sync u8, u16, u32, u64)}
        unsafe { self.from_iner(&Sync i8, i16, i32)}
